...
class PoliticalPage extends StatefulWidget {
  const PoliticalPage({super.key});

  @override
  State<PoliticalPage> createState() => _PoliticalPageState();
}
                      ...
                      child: ListView.builder(
                        //controller: scrollcontroller,
                         // physics: NeverScrollableScrollPhysics(),
                          itemCount: state.politicalnews.articles!.length,
                                  ...
                                  child: GestureDetector(
                                    onTap: (){
                                      Navigator.push(context, MaterialPageRoute(builder: (_)=>InformationScreen(
                                        title: state.politicalnews.articles![index].title.toString(),
                                        description: state.politicalnews.articles![index].description.toString(),
                                        Content:state.politicalnews.articles![index].content.toString() ,
                                        Image: state.politicalnews.articles![index].urlToImage.toString(),
                                        authorname: state.politicalnews.articles![index].author.toString(),)));
                                    },
                                    child: Column(
                                      children: [
                                        ClipRRect(
                                          borderRadius: BorderRadius.circular(10),
                                          child: state.politicalnews.articles![index].urlToImage == null ? const Image(image: AssetImage(ImageAssets.defaultimage,),fit: BoxFit.fill) :
                                          Image(
                                            image:
                                            NetworkImage(state.politicalnews.articles![index].urlToImage.toString()),
                                            fit: BoxFit.fill,
                                          )
                                        ),
                                        SizedBox(
                                            ...
                                            width: screenSize.width,
                                            child: Text(
                                              state.politicalnews.articles![index].title.toString(),
                                              style: UpdateStyle.CustomeTextStyle(
                                                  screenSize.width / 20,
                                        ...
                                        Row(
                                          ...
                                          children: [
                                            Text(state.politicalnews.articles![index].publishedAt!.toUtc().toString(),
                                                style: UpdateStyle.CustomeTextStyle(
                                                    screenSize.width / 25,
                                                    FontWeightManager.regular,
                                                    ColorManager.gray)),
